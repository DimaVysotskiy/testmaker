...
@ai_tools_router.get(
        path="/tools-info",
        summary="Получение информации о доступных AI инструментах.",
...
@ai_tools_router.post(
        "/how_llm_see_my_lecture", 
        summary="Конвертация лекции в md через LLM.",
        dependencies=[Depends(require_roles(UserRole.TEACHER, UserRole.ADMIN))]
        )
...
@ai_tools_router.post(
        "/make_test", 
        summary="Создание теста по лекции через LLM.",
        dependencies=[Depends(require_roles(UserRole.TEACHER, UserRole.ADMIN))]
        )
async def make_test(
